...
// wet paint that dries. The fallback viewport-snapshot pipeline is ported
// from our vendored Asciify so the painting still renders in browsers
// without drawElementImage. Import paths adapted to this repo.
    ...
    if (preservesNewlines) {
      ...
      for (const part of raw.split("\n")) {
        ...
        const text = transform(
          preservesSpaces ? part : part.replaceAll(/\s+/g, " ").trim()
        )
        drawAcrossRects(text, [...range.getClientRects()])
      }
      continue
    }

    const text = transform(raw.replaceAll(/\s+/g, " ").trim())
    if (!text) continue
...
function createCanvasPainting(
  ...
  try {
    return initializeCanvasPainting(elements, options)
  } catch (error) {
    console.error("Canvas painting initialization failed:", error)
    return null
  }
}

function initializeCanvasPainting(
  elements: CanvasElements,
  options: CanvasOptions
): CanvasInstance | null {
  ...
  function captureFallback() {
    ...
    try {
      ...
      if (!destroyed && !fallbackErrorLogged) {
        ...
        console.warn(
          "Canvas painting could not capture its HTML fallback:",
          error
        )
      }
    }
...
export interface CanvasPaintingProps extends CanvasOptions {
  children: ReactNode
  className?: string
...
export function CanvasPainting({
  ...
  useEffect(() => {
    ...
    instanceRef.current = createCanvasPainting(
