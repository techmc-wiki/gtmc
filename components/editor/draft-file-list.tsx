      ...
      <FileListContainer>
        {files.map((file, index) => (
          <DraftFileListItem
            ...
            isUnsaved={unsavedFileIds?.has(file.id) ?? false}
            showRemoveButton={!isReadOnly && files.length > 1}
...
function DraftFileListItem({
  ...
  file: DraftFileListProps["files"][number]
  index: number
  isActive: boolean
  isUnsaved: boolean
  ...
  const secondaryAction = React.useMemo(
    ...
    [showRemoveButton, file.id, isActive, onRemoveFile, t]
  )
